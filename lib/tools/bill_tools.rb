    ...
    def self.register(server, qb_client)
      ...
      server.define_tool(
        ...
        input_schema: {
          properties: {
            bill: {
              type: 'object',
              description: 'Bill data to update (must include id and sync_token)'
            }
          },
          required: ['bill']
        }
        ...
        begin
          ...
          bill = ::Quickbooks::Model::Bill.new(args[:bill])
          result = service.update(bill, sparse: true)
      ...
      server.define_tool(
        ...
        begin
          ...
          query_result = Helpers::SearchCriteriaBuilder.build(args, 'Bill')
